...
import com.example.weatherapp.city_manager.CityManagerActivity;
...
//  创建Fragment对象，添加到ViewPager数据源当中
    ...
    //页面重新更改时会调用的函数，这个函数是在页面获取焦点之前进行调用的，此处完成ViewPager页数的更新
    @Override
    protected void onRestart() {
        ...
        //剩下的城市也要创建对应的fragment界面
        fragmentList.clear();
        initPager();
        adapter.notifyDataSetChanged();
        //页面数量发生改变，指示器的数量也会发生变化，重写设置添加指示器
        imgList.clear();
        pointLayout.removeAllViews();  //将布局当中所有的元素全部移除
        initPoint();
        mainVp.setCurrentItem(fragmentList.size()-1);
    }
}
